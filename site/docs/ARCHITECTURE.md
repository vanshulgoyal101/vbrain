...
The same notes drive two very different sites, and the split is deliberate:

| | Private brain | Public site |
|---|---|---|
| Served by | Cloudflare **Worker** (`src/worker.js`) | Cloudflare **Pages**, static |
| Auth | Supabase Google sign-in, one email | none |
| Rendering | SPA, markdown in the browser (`public/app.js`) | pre-rendered HTML, zero JS |
| Robots | `noindex, nofollow` | `index, follow` |
| Built by | `wrangler deploy` | `build-site.mjs` → `dist/`, published by CI |

The public site is generated by **`src/ssg.js`** (pure: URLs, meta, sitemap, feed,
robots/headers, JSON-LD, heading ids + ToC, section hubs, the HTML shell) plus
**`build-site.mjs`** (IO + Markdown via `marked`, git dates). Both surfaces share
`public/lib.js` — one `slugify`, one `escapeHtml`, one search engine — so a link
to `#some-heading` resolves the same on either. See [PUBLIC-SITE.md](PUBLIC-SITE.md).
